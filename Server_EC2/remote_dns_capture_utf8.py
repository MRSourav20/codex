...
class DNSPacketCapture:
    ...
    def _handle_shutdown(self, signum, frame):
        ...
        self.running = False
        # Do NOT call sys.exit() here ΓÇö it causes SystemExit inside asyncio event loop,
    ...
    def _get_all_dns_fields(self, dns_layer) -> dict:
        ...
        attributes ΓÇö so dir() / hasattr() / getattr() don't find them.
    ...
    def _detect_dns_field(self, dns_layer) -> str:
        ...
        if not raw:
            logger.warning("DNS layer has no _all_fields ΓÇö cannot detect field.")
            return ''
        ...
        for candidate in candidates:
            if candidate in raw:
                logger.info(f"Γ£ô Detected DNS query field key: '{candidate}'")
                return candidate

        # Fallback: search raw keys for 'qry' or 'query' or 'name'
        for key in raw:
            if 'qry' in key or ('query' in key and 'name' in key):
                logger.info(f"Γ£ô Auto-detected DNS field key: '{key}'")
                return key
    ...
    def _extract_dns_domains(self, packet) -> list:
        ...
        try:
            ...
            # Pull the value from the raw dict
            value = raw.get(self.detected_dns_field)
            if not value:
                return domains

            # Value can be a single name or comma-separated list (multiple queries)
            ...
            for name in names:
                ...
                if sanitized:
                    ...
                    if self.debug:
                        logger.debug(f"Extracted domain: {sanitized}")
    ...
    def start_capture(self):
        ...
        try:
            ...
            pass  # Graceful exit ΓÇö statistics already printed via _handle_shutdown
        except Exception as e:
